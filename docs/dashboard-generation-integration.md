...
求解器的版式规则：emphasis 映射为 compact 3 列、normal 6 列、wide 8 列；hero 独占一个横向带，其后最多两个 `placement: "rail"` 的同分区组件进入侧轨垂直堆叠，底缘与 hero 严格对齐。`section` 按 summary → main → detail 纵向成带，带间留白大于带内行距；同一行内的组件高度统一、底缘对齐；不足一行的窄卡优先拉伸，但 KPI 卡封顶 4 列，不会拉成空荡的宽卡。同一 `moduleId` 的组件在分区内聚类相邻。可选 `heightTier`（slim 216 / short 280 / tall 416）表达高度意图，显式档位不会被行高统一覆盖。间距随画布宽度缩放：画布 < 1680 时边距 24 / 行距 16 / 带距 32，≥ 1680 时 32 / 24 / 40，全部保持 8px 网格对齐。
