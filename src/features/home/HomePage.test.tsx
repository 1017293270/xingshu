...
describe("HomePage", () => {
  ...
  it("uses the colorful recommended app icons and exposes selection state", async () => {
    ...
    // 3×2 排布靠固定 6 张：多一张就会退化成 4+2，跟命令框那条轴对不齐
    expect(container.querySelectorAll(".xs-app-card")).toHaveLength(6);
    expect(container.querySelectorAll('[data-icon-source="xingshu-home-apps-image2-v1"]')).toHaveLength(6);
    expect(screen.queryByRole("button", { name: /打开 智享大屏/ })).not.toBeInTheDocument();
