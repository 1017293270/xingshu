...
describe("AppRoutes", () => {
  ...
  it("keeps the shared sidebar on routed pages", async () => {
    ...
    expect(screen.getByRole("button", { name: "新建对话" })).not.toHaveClass("xs-sidebar__new-chat--current");
  ...
  it("starts an agent analysis from the home command box", async () => {
    ...
    expect(await screen.findByText("分析本月经营数据")).toBeInTheDocument();
  });

  it("stays on home and switches the ask workspace from a recommended card", async () => {
    const user = userEvent.setup();
    renderRoute("/");
  ...
  it("submits from the workspace with the selected DataHub model", async () => {
    ...
    expect(await screen.findByText("查询最新销售制度")).toBeInTheDocument();
  ...
  it("opens the internal dashboard designer from the dashboard page", async () => {
    ...
    await user.type(await screen.findByLabelText("看板名称"), "路由回归看板");
